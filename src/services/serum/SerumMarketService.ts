...
export class SerumMarketService {
    ...
    getMarkets = (mintAddresses: PublicKey[], commitment: Commitment = 'single'): Promise<Record<string, SerumMarket>> => {
        ...
        return this.client.connection.getMultipleAccountsInfo(marketAccountAddresses, commitment)
            .then(accounts => {
            ...
            .then(marketAccountsByAddress => {
                const marketAccountsByAddressWithInfo = new Map<string, { marketAccount: MarketAccount, marketInfo: MarketInfo }>();
    ...
    private getMarketData = (marketAccountsByAddress: Map<string, { marketAccount: MarketAccount, marketInfo: MarketInfo }>, commitment: Commitment): Promise<Record<string, SerumMarket>> => {
        return this.getMarketSubaccounts(marketAccountsByAddress, commitment)
            .then(marketInfoSubaccounts => {
                ...
                marketAccountsByAddress.forEach((marketAccountAndInfo, key) => {
                    ...
                    // @ts-ignore
                    const mintBase = marketAccountParser(marketAccount.data.baseMint, marketInfoSubaccounts.get(marketAccount.data.baseMint.toBase58()));
                    // @ts-ignore
                    const mintQuote = marketAccountParser(marketAccount.data.quoteMint, marketInfoSubaccounts.get(marketAccount.data.quoteMint.toBase58()));
    ...
    private getMarketSubaccounts = (marketAccountsByAddress: Map<string, { marketAccount: MarketAccount, marketInfo: MarketInfo }>, commitment: Commitment): Promise<Map<string, AccountInfo<Buffer>>> => {
        ...
        return this.fetchMarketSubaccounts(Array.from(toQuery.values()).map(val => new PublicKey(val)), commitment)
    }

    private fetchMarketSubaccounts = (subaccountAddresses: PublicKey[], commitment: Commitment): Promise<Map<string, AccountInfo<Buffer>>> => {
        return this.client.connection.getMultipleAccountsInfo(subaccountAddresses, commitment)
            .then(subaccounts => {
                return subaccountAddresses.filter((_, index) => subaccounts[index] !== null)
